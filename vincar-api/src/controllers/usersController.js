import { usersService } from "../models/users.module.js";
import Joi from "joi";
...
class UsersController {
    ...
    validateUser(data) {
        const schema = Joi.object({
            ...
            phone: Joi.string().pattern(/^\d{10,11}$/).required()
                .messages({
                    "string.pattern.base": "O telefone deve conter 10 ou 11 dígitos numéricos.",
    ...
    async createUser(req, res, next) {
        try {
            // Desestruturação de dados recebidos no body da requisição
